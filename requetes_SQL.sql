...
-- Insert data into Demande_Annulation
INSERT INTO Demande_Annulation (session_id, professeur_id, date_demande, motif, statut) VALUES
(1, 1, '2024-10-01 10:00:00', 'Problème technique', 'en_attente'),
(2, 2, '2024-10-02 11:00:00', 'Conflit de planning', 'approuvée'),
(3, 3, '2024-10-03 12:00:00', 'Maladie', 'rejetée');


SELECT id FROM Cours;

SET @cours_ids = (SELECT GROUP_CONCAT(id) FROM Cours);
SET @jours = 'Monday,Tuesday,Wednesday,Thursday,Friday';
SET @heures = '09:00:00,10:00:00,11:00:00,13:00:00,14:00:00';

-- Convertir les heures en tableau
SET @heure_start = '09:00:00';
SET @heure_end = '15:00:00';

-- Nombre de jours à ajouter (par semaine)
SET @jours_count = 5;

-- Nombre de semaines à ajouter
SET @semaines_count = 4;

-- Nombre de sessions par cours
SET @sessions_per_cours = 3;

-- Insertion des sessions aléatoires
INSERT INTO Sessions (cours_id, date, heure_debut, heure_fin)
SELECT
    cours_id,
    DATE_ADD(CURDATE(), INTERVAL (FLOOR(RAND() * @jours_count) + 1) DAY + INTERVAL (FLOOR(RAND() * @semaines_count) * 7) DAY) AS date,
    TIME_FORMAT(CONCAT(FLOOR(RAND() * (TIME_TO_SEC(@heure_end) - TIME_TO_SEC(@heure_start)) / 3600) + 9, ':00:00'), '%H:%i:%s') AS heure_debut,
    TIME_FORMAT(CONCAT(FLOOR(RAND() * (TIME_TO_SEC(@heure_end) - TIME_TO_SEC(@heure_start)) / 3600) + 10, ':00:00'), '%H:%i:%s') AS heure_fin
FROM
    (SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(@cours_ids, ',', n), ',', -1) AS cours_id
     FROM (SELECT @rownum := @rownum + 1 AS n FROM information_schema.columns, (SELECT @rownum := 0) AS init LIMIT 10) AS rows
     WHERE n <= @sessions_per_cours
    ) AS cours_sessions;


-- Exemple de données : insérer des sessions aléatoires pour les cours
SET @jours_count = 5; -- Nombre de jours dans la semaine
SET @semaines_count = 4; -- Nombre de semaines à insérer
SET @heures = '09:00:00,10:00:00,11:00:00,13:00:00,14:00:00'; -- Horaires disponibles

-- Convertir les heures en tableau
SET @heure_start = '09:00:00';
SET @heure_end = '15:00:00';

-- Insérer des sessions aléatoires
INSERT INTO Sessions (cours_id, date, heure_debut, heure_fin)
SELECT
    cours_id,
    DATE_ADD(CURDATE(), INTERVAL FLOOR(RAND() * @semaines_count * 7) DAY) AS date,
    TIME_FORMAT(CONCAT(FLOOR(RAND() * 6) + 9, ':00:00'), '%H:%i:%s') AS heure_debut,
    TIME_FORMAT(CONCAT(FLOOR(RAND() * 6) + 10, ':00:00'), '%H:%i:%s') AS heure_fin
FROM
    (SELECT id AS cours_id FROM Cours) AS cours_sessions
ORDER BY RAND()
LIMIT 10; -- Limiter le nombre d'inserts pour éviter trop de données
...
(1, '2024-07-25', '08:00:00', '10:00:00'),
(2, '2024-07-26', '10:00:00', '12:00:00');
...
-- Récupérer les IDs des étudiants insérés
SET @etudiantId1 = (SELECT id FROM Etudiants WHERE email = 'aissatou.sow@example.com');
SET @etudiantId2 = (SELECT id FROM Etudiants WHERE email = 'mamadou.ba@example.com');
...
INSERT INTO Demande_Annulation (session_id, professeur_id, date_demande, motif, statut) VALUES
(1, 5, '2024-10-01 10:00:00', 'Réunion urgente', 'en_attente'),
(2, 5, '2024-10-03 11:00:00', 'Problème de santé', 'en_attente'),
(3, 5, '2024-10-05 12:00:00', 'Conférence', 'en_attente');
...
-- Insertion dans absences
INSERT INTO absences (etudiant_id, session_id, date_absence) VALUES
(4, 4, '2024-09-11'),
(4, 6, '2024-09-13');

-- Insertion dans Demande_Annulation
INSERT INTO Demande_Annulation (session_id, professeur_id, date_demande, motif, statut) VALUES
(1, 1, '2024-10-01 10:00:00', 'Réunion urgente', 'en_attente'),
(2, 1, '2024-10-03 11:00:00', 'Problème de santé', 'en_attente'),
(3, 2, '2024-10-05 12:00:00', 'Conférence', 'en_attente');


ALTER TABLE Etudiants_Cours ADD COLUMN date_affectation DATE;

-- Définir la date actuelle pour cette semaine
SET @date_semaine = CURDATE(); -- Utiliser CURDATE() pour la date actuelle ou spécifiez une date fixe

-- Insérer des affectations pour cette semaine
...
INSERT INTO Sessions (cours_id, date, heure_debut, heure_fin, nombre_heures) VALUES
(1, '2024-08-01', '08:00:00', '10:00:00', 2),
(1, '2024-08-03', '10:00:00', '12:00:00', 2),
(2, '2024-08-02', '14:00:00', '16:00:00', 2),
(3, '2024-08-05', '08:00:00', '10:00:00', 2),
(4, '2024-09-01', '09:00:00', '11:00:00', 2),
(5, '2024-09-02', '13:00:00', '15:00:00', 2);
...
SELECT id, libelle FROM Cours;


SELECT * FROM Sessions WHERE date BETWEEN '2024-07-01' AND '2024-08-31';
...
SELECT * FROM `Professeurs_Cours`

SELECT * FROM `Demande_Annulation`;

desc `Demande_Annulation`
...
ADD demande_annulation BOOLEAN DEFAULT FALSE;

SHOW COLUMNS FROM Sessions;



--------------------------------------------Organisation et commentaire de la base de données-----------------------------------------
...
INSERT INTO Demande_Annulation (session_id, professeur_id, date_demande, motif, statut) VALUES
(1, 1, '2024-10-01 10:00:00', 'Réunion urgente', 'en_attente'),
(2, 2, '2024-10-03 11:00:00', 'Problème de santé', 'en_attente'),
(3, 3, '2024-10-05 12:00:00', 'Conférence', 'en_attente');
...
-- Justifications
INSERT INTO justifications (absence_id, motif, piece_jointe) VALUES
(1, 'Malade', 'certificat_medical.pdf'),
(2, 'Problème familial', 'justification_parentale.pdf');
...
(2, '2024-08-10', '13:00:00', '15:00:00'),
(1, '2024-08-11', '09:00:00', '11:00:00'),
(2, '2024-08-12', '10:00:00', '12:00:00'),
(1, '2024-08-13', '11:00:00', '13:00:00'),
(2, '2024-08-14', '12:00:00', '14:00:00'),
(1, '2024-08-15', '13:00:00', '15:00:00'),
(2, '2024-08-16', '09:00:00', '11:00:00'),
(1, '2024-08-17', '10:00:00', '12:00:00'),
(2, '2024-08-18', '11:00:00', '13:00:00'),
(1, '2024-08-19', '12:00:00', '14:00:00'),
(2, '2024-08-20', '13:00:00', '15:00:00'),
(1, '2024-08-21', '09:00:00', '11:00:00'),
(2, '2024-08-22', '10:00:00', '12:00:00'),
(1, '2024-08-23', '11:00:00', '13:00:00'),
(2, '2024-08-24', '12:00:00', '14:00:00'),
(1, '2024-08-25', '13:00:00', '15:00:00'),
(2, '2024-08-26', '09:00:00', '11:00:00'),
(1, '2024-08-27', '10:00:00', '12:00:00'),
(2, '2024-08-28', '11:00:00', '13:00:00'),
(1, '2024-08-29', '12:00:00', '14:00:00'),
(2, '2024-08-30', '13:00:00', '15:00:00'),
(1, '2024-08-31', '09:00:00', '11:00:00');
